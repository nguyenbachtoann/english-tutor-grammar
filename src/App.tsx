...
// --- AI SERVICE (PROXY) ---
const callGemini = async (prompt: string, systemInstruction: string = "", jsonMode: boolean = false): Promise<{text: string, error?: string}> => {

  const makeRequest = async (retryCount = 0): Promise<{text: string, error?: string}> => {
    try {
      ...
      // If offline or network error, fail faster or handle specifically
      ...
      if (retryCount < 2) {
        // Exponential backoff for server waking up
        await new Promise(r => setTimeout(r, 2000 * (retryCount + 1)));
        return makeRequest(retryCount + 1);
      }
      return { text: "", error: "SERVER_ERROR" };
    }
  };
...
const LessonView: React.FC<LessonViewProps> = ({ topic, level, theme, onBack, onComplete }) => {
  ...
  const generateLesson = useCallback(async (retryCount = 0) => {
    ...
    try {
      const { text, error } = await callGemini(prompt, "", true);
  ...
  if (loading) return (
    <div className="h-full flex flex-col items-center justify-center space-y-4 bg-white pt-20">
      ...
      <p className="text-gray-500 font-medium">Đang kết nối Server & soạn bài...</p>
  ...
  if (errorType) return (
    <div className="p-6 text-center pt-20 flex flex-col items-center">
      ...
      <h3 className="text-lg font-bold text-gray-800">
        {errorType === 'OFFLINE' ? 'Mất kết nối Internet' : 'Không thể kết nối Server'}
      </h3>

      <p className="text-gray-500 mb-6 px-4">
        {errorType === 'OFFLINE'
          ? 'Vui lòng kiểm tra lại đường truyền mạng của bạn.'
          : 'Server Render miễn phí có thể đang khởi động. Hãy thử lại sau ít phút.'}
      </p>
...
const ChatTutor: React.FC<ChatTutorProps> = ({ level, theme }) => {
  ...
  const sendMessage = async () => {
    ...
    setMessages(prev => [...prev, { role: 'ai', text: text || "Đang mất kết nối với Server..." }]);
    setTyping(false);
  };
...
const WritingLab: React.FC<WritingLabProps> = ({ level, theme }) => {
  ...
  const analyzeText = async () => {
    ...
    try {
      const { text: response } = await callGemini(prompt, "", true);
      if (!response) {
      ...
      setAnalysis({
        isCorrect: false,
        corrected: "Mất kết nối",
        analysis: "Vui lòng kiểm tra lại mạng hoặc server backend."
      });
    } finally {
