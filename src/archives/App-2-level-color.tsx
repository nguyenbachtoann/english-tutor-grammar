...
/* GRAMMARFLOW - DYNAMIC THEME VERSION
  ------------------------------
  Updates:
  - Implemented Level-based Theming (Emerald for Beginner, Blue for Intermediate, Rose for Advanced).
  - Dynamic colors for all components based on current level.
...
interface ThemeConfig {
  id: string;
  primary: string;      // Main background color (e.g., header)
  secondary: string;    // Light background for cards
  text: string;         // Primary text color
  textLight: string;    // Lighter text color
  border: string;       // Border color
  gradient: string;     // Gradient for special buttons/cards
...
const LessonView: React.FC<LessonViewProps> = ({ topic, level, theme, onBack, onComplete }) => {
  ...
  const generateLesson = useCallback(async (retryCount = 0) => {
    ...
    const prompt = `
      Đóng vai một giáo viên tiếng Anh dạy cho học sinh trình độ ${level}.
      Soạn bài học về "${topic.title}".

      Yêu cầu đầu ra (JSON):
      {
        "title": "Tiêu đề (Anh/Việt)",
        "intro": "Giới thiệu ngắn (Tiếng Việt)",
        "theory": "Lý thuyết ngữ pháp (Tiếng Việt, dùng markdown)",
        "examples": ["Ví dụ 1 (Anh)", "Ví dụ 2 (Anh)"],
        "quiz": {
          "question": "Câu hỏi trắc nghiệm (Anh)",
          "options": ["A", "B", "C"],
          "correct": 0,
    ...
    try {
      const text = await callGemini(prompt, "Responds in JSON with Vietnamese explanations.");
      ...
      if (retryCount < 2) {
        setTimeout(() => generateLesson(retryCount + 1), 1500);
      } else {
        setLoading(false);
        setError(true);
      }
  ...
  if (loading) return (
    <div className="h-full flex flex-col items-center justify-center space-y-4 bg-white pt-20">
      ...
      <p className="text-gray-500 font-medium">Đang soạn bài giảng...</p>
    </div>
  );
  ...
  if (error || !content) return (
    <div className="p-6 text-center pt-20 flex flex-col items-center">
      ...
      <h3 className="text-lg font-bold text-gray-800">Mất kết nối</h3>
...
const DailyChallenge: React.FC<DailyChallengeProps> = ({ level, theme, onClose, onComplete }) => {
  ...
  const generateDailyQuestion = useCallback(async (retryCount = 0) => {
    ...
    const prompt = `Tạo 1 câu hỏi trắc nghiệm ngữ pháp tiếng Anh trình độ ${level}.
    Trả về JSON:
    {
      "question": "Câu hỏi (Anh)",
      "options": ["A", "B", "C", "D"],
      "correct": 0,
      "explanation": "Giải thích (Việt)"
    }`;

    try {
      const text = await callGemini(prompt, "API JSON.");
...
const ChatTutor: React.FC<ChatTutorProps> = ({ level, theme }) => {
  ...
  const sendMessage = async () => {
    ...
    const prompt = `
      ...
      Student: ${userMsg}
      Level: ${level}.
      Explain concepts in VIETNAMESE. Use English for examples.
    `;

    const reply = await callGemini(prompt, "English tutor explaining in Vietnamese.");

    setMessages(prev => [...prev, { role: 'ai', text: reply || "Lỗi kết nối. Thử lại nhé." }]);
    setTyping(false);
  };
...
const WritingLab: React.FC<WritingLabProps> = ({ level, theme }) => {
  ...
  const analyzeText = async () => {
    ...
    const prompt = `Phân tích câu: "${text}". Level: ${level}.
    JSON Output: { "corrected": "...", "isCorrect": boolean, "analysis": "Vietnamese md", "betterWay": "..." }`;

    try {
      const response = await callGemini(prompt, "Grammar expert JSON.");
      ...
      setAnalysis({
        isCorrect: false,
        corrected: "Lỗi phân tích",
        analysis: "Vui lòng thử lại với câu ngắn hơn."
      });
    }
    ...
    <div className="p-4 space-y-4">
      <div className={`bg-gradient-to-r ${theme.gradient} p-6 rounded-2xl mb-4 shadow-lg text-white`}>
        <h3 className="font-bold flex items-center gap-2 text-lg">
          <Edit3 className="w-5 h-5" /> Phòng Lab Viết
        </h3>
        ...
        <div className="animate-fade-in-up bg-white border border-gray-100 shadow-xl rounded-2xl overflow-hidden mt-6">
          <div className={`p-4 ${analysis.isCorrect ? 'bg-green-500' : 'bg-amber-500'} text-white flex items-center gap-2`}>
            ...
            <span className="font-bold text-lg">
              {analysis.isCorrect ? 'Tuyệt vời!' : 'Cần cải thiện'}
            </span>
          </div>
...
export default function App() {
  ...
  // Derive current theme from level
